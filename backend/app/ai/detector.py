...
class FaceDetector:
    ...
    def load(self) -> None:
        """Download (first run) and prepare the SCRFD detector + ArcFace model."""
        from insightface.app import FaceAnalysis

        logger.info(
            "FaceDetector: loading %s from %s", self.model_name, MODELS_DIR
        )
        self.app = FaceAnalysis(name=self.model_name, root=str(MODELS_DIR))
